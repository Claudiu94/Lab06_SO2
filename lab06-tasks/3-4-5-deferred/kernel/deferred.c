...
static void timer_handler(unsigned long var)
{
	 ...
	 */
	else if (dev.flag == TIMER_TYPE_ACCT && current->pid == 0) {
		spin_lock(&dev.lock);
		if (dev.buf_idx < BUFFER_LEN) {
			dev.buf[dev.buf_idx] = current->nivcsw;
			dev.buf_idx += 1;
		}
